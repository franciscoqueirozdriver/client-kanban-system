...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Adiciona faltantes preservando ordem atual
    for (const c of neededCols) {
      if (!header.includes(c)) header.push(c);
    }
    // Se header mudou, reescrever linha 1
    ...
    for (const row of layout) {
      ...
      if (!base) { ignored++; continue; } // sem referência na Sheet1
      ...
      // Produto (apenas se constar em Padroes/Produtos)
      ...
      // Observação
      const observacao = clean(row['Observação']); // manter vazio se vier vazio
      ...
      // Monta objeto final (não sobrescrever com vazio)
      const novo = {
        [KEY]: clientId,
