...
export async function enrichCompanyData(empresa) {
  ...
  // Se houver CEP e dados de endereço faltando, consulta a API ViaCEP
  if (
    enriched.cep &&
    (!enriched.logradouro || !enriched.bairro || !enriched.cidade || !enriched.estado || !enriched.complemento)
  ) {
    try {
      const cepDigits = String(enriched.cep).replace(/\D+/g, '');
      ...
      if (viaCepRes.ok) {
        ...
        if (!viaCepData.erro) {
          ...
          enriched.complemento ||= viaCepData.complemento || '';
        }
      }
    } catch {
