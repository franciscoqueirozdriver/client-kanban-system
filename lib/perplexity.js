...
// - Se não houver telefone(s), DDI pode ficar em branco.
//
// Campos suportados no fluxo interno:
...
// ==================== Prompt (custom do cliente) ====================
function buildCNPJPrompt(empresa) {
  const nome = norm(empresa?.nome);

  // Prompt solicitado pelo cliente (“Melhor Prompt para Consulta Completa”)
  return `
...
async function callPerplexityJSON(prompt, signal) {
  ...
  // Extrai o primeiro objeto JSON (se vier com algum ruído)
...
export async function enrichCompanyData(empresa) {
  ...
  if (!fixedCNPJ) {
    // Sem CNPJ, não falhamos o fluxo — devolvemos o que já tínhamos, sanitizado.
