// lib/perplexity.js
// Versão com LOGS detalhados controlados por DEBUG_PERPLEXITY=1
// Estratégia de robustez:
// 1) Prompt pede TABELA (Markdown).
// 2) Parse da tabela -> objeto.
// 3) Fallback: extrair CNPJ via regex quando não houver tabela.
// 4) Fallback: 2ª chamada pedindo apenas o CNPJ da MATRIZ (0001).
// 5) DDI = '55' apenas se houver telefone; senão, vazio.
// 6) Timeout: PERPLEXITY_TIMEOUT_MS (default 10000).
...
const DEBUG = process.env.DEBUG_PERPLEXITY === '1';
...
function errlog(...a){ console.error(tag, ...a); }
function safe(v, max=1600){
...
function parseMarkdownTableToObject(md) {
  ...
  log('tabela.header.idx', headerLineIdx);

  if (headerLineIdx < 0) {
    warn('tabela.nao_encontrada');
    return null;
  }
  ...
  if (dataStart >= lines.length) {
    warn('tabela.sem_linha_de_dados');
    return null;
  }
...
async function callPerplexity(prompt, signal) {
  ...
  // (1) LOG do conteúdo bruto da resposta
  log('api.resposta.bruta', safe(content));
...
export async function enrichCompanyData(empresa) {
  ...
  if (fixedAlready) {
    ...
    log('final.com_cnpj_preexistente', sanitized);
    return sanitized;
  }
  ...
  try {
    ...
    // 2) Validar parser da tabela Markdown
    const parsed = parseMarkdownTableToObject(content);
    if (parsed) {
      enriched = onlyAllowedKeys(parsed);
    } else {
      // 3) Fallback: extração CNPJ via regex
      const cnpjFromText = extractCNPJFromText(content);
      log('regex.cnpj.from_full', cnpjFromText);
      enriched = onlyAllowedKeys({});
      if (cnpjFromText) enriched.cnpj = cnpjFromText;
    }
  } catch (err) {
    errlog('consulta.tabela.fail', err?.message || err);
    enriched = {};
  }

  // 4) Se ainda não há CNPJ, 2ª chamada “CNPJ-only”
  ...
  if (!fixedCNPJ) {
    try {
      ...
      log('api.resposta.cnpj_only', safe(content2));
      ...
      try {
        ...
        if (jsonStr) {
          ...
          log('cnpj_only.json', obj);
          fixedCNPJ = normalizeCNPJ(obj?.cnpj || '');
        }
      } catch (error) {
        warn('cnpj_only.json.parse.fail', error?.message || error);
      }
      if (!fixedCNPJ) {
        const rx = normalizeCNPJ(extractCNPJFromText(content2));
        log('cnpj_only.regex', rx);
        fixedCNPJ = rx;
      }
    } catch (err) {
      errlog('consulta.cnpj_only.fail', err?.message || err);
    }
  }

  if (!fixedCNPJ) {
    const sanitized = sanitizeBrazilFields(base);
    log('final.sem_cnpj', sanitized);
    return sanitized;
  }
  ...
  // 5) Log do retorno final já normalizado
  log('final.com_cnpj', sanitized);
