...
export function normalizaListaTelefones(s) {
  if (!s) return null;
  const parts = s.split(";").map(x => x.trim()).filter(Boolean);
  if (parts.length === 0) return null;
  const norm = parts.map(t => normalizaTelefoneBR(t));
  return norm.join(";");
}

// Opcional: obter $metadata para confirmar o EntitySet
export async function getMetadata() {
  const url = joinUrl(BASE_URL, '$metadata');
...
export async function createOportunidadeSpotter(data, { entitySetPath = '/Oportunidades' } = {}) {
  const url = joinUrl(BASE_URL, entitySetPath);
