...
export async function findRowIndexById(sheetName, headersRow, idColumnName, idValue) {
  ...
  // Tenta encontrar a coluna original ou sua versão snake_case
  let idCol = idColumnName;
  if (!headers.includes(idCol)) {
    const snake = idCol.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
    if (headers.includes(snake)) {
      idCol = snake;
    } else if (idCol === 'Cliente_ID' && headers.includes('cliente_id')) {
      idCol = 'cliente_id';
    }
  }
  ...
  for (const row of rows) {
    const cell = String(row[idCol] || row[idColumnName] || '').trim();
...
export async function updateRowByIndex({ sheetName, rowIndex, updates }) {
  ...
  Object.entries(updates || {}).forEach(([col, value]) => {
    ...
    if (idx === undefined) {
      const snake = col.toLowerCase().replace(/[^a-z0-9_]+/g, '_').replace(/^_|_$/g, '');
      idx = headerMap[snake];
      if (idx === undefined && col === 'Cliente_ID') idx = headerMap['cliente_id'];
    }
...
export async function _findRowNumberByClienteId(sheetName, clienteId) {
    ...
    const rowIndex = rows.findIndex(row => (row.cliente_id || row['Cliente_ID']) === clienteId);
...
export async function getNextClienteId() {
  readCache.delete(`sheetData:leads_exact_spotter`);
  const { rows } = await getSheetData('leads_exact_spotter');
  let maxId = 0;
  for (const row of rows) {
...
const SHEETS_TO_SEARCH = ['leads_exact_spotter', 'layout_importacao_empresas', 'sheet1'];
...
export async function findByCnpj(cnpj) {
  ...
  for (const sheetName of SHEETS_TO_SEARCH) {
    ...
    const { rows } = await getSheetData(sheetName);
    for (const row of rows) {
      const rowCnpj = row['cnpj_empresa'] || row['CNPJ Empresa'] || row['cpf_cnpj'] || row['CPF/CNPJ'];
...
export async function findByName(name) {
  ...
  for (const sheetName of SHEETS_TO_SEARCH) {
    ...
    const { rows } = await getSheetData(sheetName);
    for (const row of rows) {
      const rowName = row['nome_da_empresa'] || row['Nome da Empresa'] || row['nome_do_lead'] || row['Nome do Lead'];
      if (normalizeText(rowName) === normalizedNameToFind) {
        const rowCnpj = normalizeCnpj(row['cnpj_empresa'] || row['CNPJ Empresa'] || row['cpf_cnpj'] || row['CPF/CNPJ']);
        if (!rowCnpj) {
...
export async function appendToSheets(payload) {
    ...
    const sheetProcessOrder = [
        { name: 'leads_exact_spotter', builder: buildLeadsExactSpotterRow },
...
export async function updateRow(rowNumber, data) {
    ...
    for (const key in data) {
        ...
        if (colIndex === undefined) {
            ...
            colIndex = headerMap[snake];
            if (colIndex === undefined && key === 'Cliente_ID') colIndex = headerMap['cliente_id'];
        }
...
export async function appendHistoryRow(data) {
    ...
    const row = headers.map(header => {
        ...
        // Fallback para chaves com formatos diferentes no objeto 'data'
        const keys = Object.keys(data);
        const match = keys.find(k => {
            const normalizedK = k.toLowerCase().replace(/[^a-z0-9_]+/g, '_').replace(/^_|_$/g, '');
            return normalizedK === header;
        });
...
export async function updateInSheets(clienteId, payload) {
    ...
    const sheetBuilders = {
        'leads_exact_spotter': buildLeadsExactSpotterRow,
...
export async function getCompanySheet() {
  return getSheet(COMPANY_IMPORT_SHEET_NAME);
}

export async function getCompanySheetCached() {
  return getSheetCached(COMPANY_IMPORT_SHEET_NAME);
}
...
export async function appendCompanyImportRow(data) {
  const sheets = await getSheetsClient();
  const { headers } = await getSheetData(COMPANY_IMPORT_SHEET_NAME);

  const row = headers.map(h => {
    // COMPANY_COLUMN_MAP maps keys like 'nome' to header names like 'nome'
    const key = Object.keys(COMPANY_COLUMN_MAP).find(k => COMPANY_COLUMN_MAP[k] === h);
    return key ? protectValue(data[key]) : protectValue(data[h]);
  });
