...
// Assinatura esperada pela rota: appendSheetData({ spreadsheetId, range, values, valueInputOption? })
// -----------------------------------------------------------------------------
export async function appendSheetData({ spreadsheetId, range, values, valueInputOption = 'USER_ENTERED' }) {
  ...
  // Tenta reusar um helper local, se existir (ex.: getAuth() já definido no arquivo)
  let auth;
  if (typeof getAuth === 'function') {
    auth = await getAuth();
  } else {
    // Fallback: cria JWT com variáveis de ambiente
    const clientEmail = process.env.GOOGLE_CLIENT_EMAIL;
    let privateKey = process.env.GOOGLE_PRIVATE_KEY;
    if (!clientEmail || !privateKey) {
      throw new Error('appendSheetData: defina GOOGLE_CLIENT_EMAIL e GOOGLE_PRIVATE_KEY no ambiente');
    }
    // Corrige quebra de linha escapada em env
