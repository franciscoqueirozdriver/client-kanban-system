...
import { normalizeCnpj, nextClienteId as generateNextClienteId } from './normalizers.js';
...
const SHEETS_TO_READ_CLIENTE_ID = ['Leads Exact Spotter', 'layout_importacao_empresas', 'Sheet1', 'PERDECOMP'];

async function fetchAllClienteIds() {
  const allIds = new Set();
  for (const sheetName of SHEETS_TO_READ_CLIENTE_ID) {
    try {
      ...
      console.warn(`Could not read Cliente_ID from sheet: ${sheetName}`, e);
    }
  }
...
export async function getNextClienteId() {
  return generateNextClienteId(fetchAllClienteIds);
}
