...
export async function appendToSheets(sheetName, data) {
  return appendSheetDataLegacy(sheetName, data);
}
...
export async function updateInSheets(clienteId, payload) {
  ...
  if (rowNumber === -1) {
    throw new Error(`Cliente_ID ${clienteId} não encontrado para atualização.`);
  }
  // Remove Cliente_ID do payload para não tentar atualizar a coluna de ID
  const { Cliente_ID, ...updates } = payload;
  
  // Mapear o payload para snake_case para usar updateRowByIndex
  const snakeCaseUpdates = {};
  for (const key in updates) {
    // Assumindo que as chaves de nível superior (Empresa, Contato, Comercial)
    // são objetos e precisam ser desmembradas.
    if (typeof updates[key] === 'object' && updates[key] !== null) {
      for (const subKey in updates[key]) {
        // Converte a chave para snake_case (ex: Nome_da_Empresa -> nome_da_empresa)
        const snakeKey = subKey.replace(/([A-Z])/g, "_$1").toLowerCase();
        snakeCaseUpdates[snakeKey] = updates[key][subKey];
      }
    } else {
  ...
  await updateRowByIndex({ sheetName, rowIndex: rowNumber, updates: snakeCaseUpdates });
  readCache.delete(`sheetData:${sheetName}`);
}

async function appendSheetDataLegacy(sheetName, data) {
  const spreadsheetId = process.env.SPREADSHEET_ID;
  if (!spreadsheetId) {
    throw new Error('SPREADSHEET_ID is not set');
  }
...
export async function appendRow(data) {
    ...
    return appendSheetDataLegacy(sheetName, [row]);
}
...
export async function getSheetCached(sheetName = 'Sheet1', range = 'A:ZZ') {
  const key = `sheetDataRaw:${sheetName}:${range}`;
  const cached = readCache.get(key);
  if (cached && Date.now() - cached.time < 10000) {
    return cached.data;
  }
...
// -----------------------------------------------------------------------------
// Compat shim: appendSheetData
// Adiciona linhas ao final do range informado (A1 notation).
// Assinatura: appendSheetData({ spreadsheetId, range, values, valueInputOption? })
// -----------------------------------------------------------------------------
export async function appendSheetData({ spreadsheetId, range, values, valueInputOption = 'USER_ENTERED' }) {
  if (!spreadsheetId) throw new Error('appendSheetData: spreadsheetId ausente');
  if (!range) throw new Error('appendSheetData: range ausente');
  if (!Array.isArray(values)) throw new Error('appendSheetData: values deve ser um array de arrays (linhas)');
  ...
  if (typeof getAuth === 'function') {
    ...
    if (!clientEmail || !privateKey) {
      throw new Error('appendSheetData: defina GOOGLE_CLIENT_EMAIL e GOOGLE_PRIVATE_KEY no ambiente');
    }
  ...
  const res = await sheets.spreadsheets.values.append({
    ...
    insertDataOption: 'INSERT_ROWS', // padrão
    requestBody: { values },         // [[cell1, cell2, ...], ...]
  });
