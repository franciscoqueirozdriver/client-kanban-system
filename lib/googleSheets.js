...
import { mapHeaders, resolveSheetName, toSnake } from './naming.js';
...
function makeCacheKey(sheetName, range = DEFAULT_RANGE, spreadsheetId = getDefaultSpreadsheetId()) {
  const normalizedSheet = toSnake(sheetName || '');
  return `sheetData:${spreadsheetId}:${normalizedSheet}:${range}`;
}

function makeRawCacheKey(sheetName, spreadsheetId = getDefaultSpreadsheetId()) {
  const normalizedSheet = toSnake(sheetName || '');
  return `sheet_raw:${spreadsheetId}:${normalizedSheet}`;
}

function clearSheetCache(sheetName, spreadsheetId = getDefaultSpreadsheetId()) {
  const normalizedSheet = toSnake(sheetName || '');
  for (const key of Array.from(readCache.keys())) {
    if (key.startsWith(`sheetData:${spreadsheetId}:${normalizedSheet}:`)) {
      readCache.delete(key);
    }
  }
  readCache.delete(makeRawCacheKey(sheetName, spreadsheetId));
}

function buildRangeA1(resolvedSheetName, range) {
  if (!range) return resolvedSheetName;
  if (range.includes('!')) {
    const [, suffix = ''] = range.split('!');
    return `${resolvedSheetName}!${suffix}`;
  }
  return `${resolvedSheetName}!${range}`;
}

async function getSpreadsheetTitles(spreadsheetId) {
  ...
  sheetMetaCache.set(spreadsheetId, { time: Date.now(), titles });
  return titles;
}

async function resolveSheetMeta(sheetName, spreadsheetId) {
  if (!sheetName) {
    throw new Error('sheetName is required.');
  }
  const titles = await getSpreadsheetTitles(spreadsheetId);
  let { resolved } = resolveSheetName(sheetName, titles);
  if (!resolved) {
    sheetMetaCache.delete(spreadsheetId);
    const freshTitles = await getSpreadsheetTitles(spreadsheetId);
    ({ resolved } = resolveSheetName(sheetName, freshTitles));
    if (!resolved) {
      throw new Error(`Sheet "${sheetName}" was not found in spreadsheet ${spreadsheetId}.`);
    }
    return { resolvedSheetName: resolved, titles: freshTitles };
  }
  return { resolvedSheetName: resolved, titles };
}
...
export async function getSheetData(sheetName, range = DEFAULT_RANGE, spreadsheetId = getDefaultSpreadsheetId()) {
  const targetSpreadsheetId = spreadsheetId || getDefaultSpreadsheetId();
  if (!targetSpreadsheetId) {
    throw new Error('SHEETS_ID or SPREADSHEET_ID is not set');
  }

  const cacheKey = makeCacheKey(sheetName, range, targetSpreadsheetId);
  const cached = readCache.get(cacheKey);
  if (cached && Date.now() - cached.time < 10000) {
    return cached.data;
  }

  const { resolvedSheetName } = await resolveSheetMeta(sheetName, targetSpreadsheetId);
  const sheets = await getSheetsClient();
  const res = await withRetry(() =>
    sheets.spreadsheets.values.get({
      spreadsheetId: targetSpreadsheetId,
      range: buildRangeA1(resolvedSheetName, range),
    })
  );

  const values = res.data.values || [];
  if (values.length === 0) {
    const emptyResult = { headers: [], normalizedHeaders: [], rows: [], sheetName: resolvedSheetName };
    readCache.set(cacheKey, { time: Date.now(), data: emptyResult });
    return emptyResult;
  }

  const originalHeaders = (values[0] || []).map((header) => (header || '').toString().trim());
  const normalizedHeaders = mapHeaders(originalHeaders);

  const rows = values.slice(1).map((row, idx) => {
    const obj = { _rowNumber: idx + 2 };
    originalHeaders.forEach((original, colIdx) => {
      const canonical = normalizedHeaders[colIdx] || '';
  ...
  const result = { headers: originalHeaders, normalizedHeaders, rows, sheetName: resolvedSheetName };
  readCache.set(cacheKey, { time: Date.now(), data: result });
  return result;
}
...
export async function updateRowByIndex({ sheetName, rowIndex, updates }) {
  ...
  const sheets = await getSheetsClient();
  const { headers, normalizedHeaders, rows, sheetName: resolvedSheetName } = await getSheetData(sheetName);
  const headerMap = {};
  headers.forEach((h, i) => {
    headerMap[h] = i;
    const normalized = toSnake(h);
    const canonical = normalizedHeaders[i];
  ...
  Object.entries(updates || {}).forEach(([col, value]) => {
    ...
    const range = `${resolvedSheetName}!${colLetter}${rowIndex}:${colLetter}${rowIndex}`;
  ...
  clearSheetCache(sheetName, spreadsheetId);
}

export async function _findRowNumberByClienteId(sheetName, clienteId) {
    const { rows } = await getSheetData(sheetName);
    const rowIndex = rows.findIndex(
      (row) => row.Cliente_ID === clienteId || row.cliente_id === clienteId
    );
...
export async function getNextClienteId() {
  clearSheetCache('Leads Exact Spotter');
...
export async function findByCnpj(cnpj) {
  ...
  for (const sheetName of SHEETS_TO_SEARCH) {
    ...
    const { rows } = await getSheetData(sheetName);
    for (const row of rows) {
      const rowCnpj = row['CNPJ Empresa'] || row['CPF/CNPJ'] || row.cnpj || row.cpf_cnpj;
...
export async function findByName(name) {
  ...
  for (const sheetName of SHEETS_TO_SEARCH) {
    ...
    const { rows } = await getSheetData(sheetName);
    for (const row of rows) {
      const rowName = row['Nome da Empresa'] || row['Nome do Lead'] || row.empresa_nome || row.nome_da_empresa;
      if (normalizeText(rowName) === normalizedNameToFind) {
        const rowCnpj = normalizeCnpj(row['CNPJ Empresa'] || row['CPF/CNPJ'] || row.cnpj || row.cpf_cnpj);
        if (!rowCnpj) {
...
export async function appendToSheets(payload) {
    ...
    for (const sheet of sheetProcessOrder) {
        try {
            ...
            clearSheetCache(sheet.name, spreadsheetId);
            const { resolvedSheetName } = await resolveSheetMeta(sheet.name, spreadsheetId);
            await withRetry(() =>
...
export async function appendSheetData(sheetName, rowsToAppend, spreadsheetId = getDefaultSpreadsheetId()) {
  ...
  clearSheetCache(sheetName, targetSpreadsheetId);
}
...
async function _getRawSheet(sheetName = 'Sheet1', spreadsheetId = getDefaultSpreadsheetId()) {
    const targetSpreadsheetId = spreadsheetId || getDefaultSpreadsheetId();
    if (!targetSpreadsheetId) {
        throw new Error('SHEETS_ID or SPREADSHEET_ID is not set');
    }
...
export async function getSheetCached(sheetName = 'Sheet1') {
    ...
    const key = makeRawCacheKey(sheetName, spreadsheetId);
    const cached = readCache.get(key);
    if (cached && (Date.now() - cached.time < 10000)) {
        return cached.data;
    }
    const data = await _getRawSheet(sheetName, spreadsheetId);
...
export async function updateRow(rowNumber, data) {
    ...
    const sheets = await getSheetsClient();
    const { headers, normalizedHeaders, sheetName: resolvedSheetName } = await getSheetData('Sheet1');
    const headerMap = {};
    headers.forEach((h, i) => {
        headerMap[h] = i;
        const normalized = toSnake(h);
    ...
    for (const key in data) {
        ...
        if (colIndex !== undefined) {
            ...
            const range = `${resolvedSheetName}!${colLetter}${rowNumber}`;
    ...
    if (updates.length) {
        ...
        clearSheetCache('Sheet1', spreadsheetId);
    }
}
...
export async function appendRow(data) {
    ...
    const row = columnOrder.map((header) => {
        const normalized = toSnake(header);
        const candidates = [normalized, header];
...
export async function appendHistoryRow(data) {
    const { headers, normalizedHeaders } = await getSheetData('Historico_Interacoes');
    const row = headers.map((header, idx) => {
        const canonical = normalizedHeaders[idx];
        const normalized = toSnake(header);
        const candidates = [canonical, normalized, header];
        for (const key of candidates) {
            if (key && Object.prototype.hasOwnProperty.call(data, key)) {
                return data[key];
            }
...
export async function updateInSheets(clienteId, payload) {
    ...
    for (const sheetName in sheetBuilders) {
        ...
        const { resolvedSheetName } = await resolveSheetMeta(sheetName, spreadsheetId);
        const range = `${resolvedSheetName}!A${rowNumber}:${rangeEndColumn}${rowNumber}`;
