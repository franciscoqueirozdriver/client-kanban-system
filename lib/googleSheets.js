...
// ---------- Generic helpers for arbitrary sheets ----------
async function fetchSheetByName(sheetName, spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  return sheets.spreadsheets.values.get({
    spreadsheetId,
...
export async function getSheetByNameCached(sheetName, spreadsheetId = process.env.SPREADSHEET_ID) {
  ...
  return withCache(key, () => fetchSheetByName(sheetName, spreadsheetId));
}

export async function appendRowsToSheet(sheetName, rows, spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  const protectedRows = rows.map((r) => r.map(protectValue));
