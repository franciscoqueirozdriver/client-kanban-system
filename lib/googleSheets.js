...
import { SHEETS, Sheet1Row, LayoutImportacaoEmpresasRow, LeadsExactSpotterRow, PerdecompRow, PerdcompItensRow, PerdcompFactsRow, PerdecompSnapshotRow } from '../config/sheets';
...
/**
 * Lê os dados de uma aba da planilha e retorna um array de objetos tipados.
 * @template T O tipo de linha esperado (ex: Sheet1Row).
 * @param {string} sheetName O nome da aba (deve ser um valor de SHEETS).
 * @param {string} range O range a ser lido (padrão: 'A:ZZ').
 * @param {string} spreadsheetId O ID da planilha.
 * @returns {Promise<{headers: string[], rows: T[]}>}
 */
export async function getSheetData(sheetName, range = 'A:ZZ', spreadsheetId = process.env.SPREADSHEET_ID) {
  ...
  // Adicionando um helper tipado para uso futuro (Módulo 1 em diante)
  const typedRows = data.map(row => {
    // Aqui, em módulos futuros, podemos adicionar lógica de validação de tipo
    return row;
  });

  const result = { headers, rows: typedRows };
...
/**
 * Adiciona uma nova linha à aba da planilha.
 * @template T O tipo de linha a ser adicionada.
 * @param {string} sheetName O nome da aba (deve ser um valor de SHEETS).
 * @param {T} row O objeto com os dados da linha.
...
export async function appendRow(sheetName, row) {
  ...
  // 1. Obter os cabeçalhos para garantir a ordem correta
  const { headers } = await getSheetData(sheetName, 'A1:ZZ1');

  // 2. Mapear o objeto 'row' para um array de valores na ordem dos cabeçalhos
  const values = [
    headers.map(header => {
      // Usar o nome da coluna como chave para obter o valor do objeto 'row'
      return row[header] !== undefined ? row[header] : '';
    })
  ];

  // 3. Chamar a API de append
  await withRetry(() =>
    sheets.spreadsheets.values.append({
      spreadsheetId,
      range: `${sheetName}!A:A`, // Apenas A:A para garantir que o append vá para a próxima linha
...
/**
 * Atualiza linhas existentes na aba da planilha.
 * NOTA: Esta função é um placeholder. A implementação real pode ser complexa
 * dependendo de como as linhas são identificadas (ex: por cliente_id).
 * Para o Módulo 0, vamos apenas criar a assinatura.
 * @template T O tipo de linha a ser atualizada.
 * @param {string} sheetName O nome da aba (deve ser um valor de SHEETS).
 * @param {T[]} rows O array de objetos com os dados das linhas a serem atualizadas.
 * @returns {Promise<void>}
 */
export async function updateRows(sheetName, rows) {
  console.warn(\`[updateRows] Função não implementada para o Módulo 0. Chamada para \${sheetName} com \${rows.length} linhas.\`);
  // A implementação completa de updateRows será feita nos módulos subsequentes
  // onde a lógica de identificação de linha (ex: por cliente_id) for necessária.
...
// Estes builders serão removidos/refatorados nos módulos subsequentes (Módulo 1 em diante)
// para usar os novos helpers readSheet/appendRow/updateRows e os tipos definidos.
