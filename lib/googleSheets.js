...
export async function appendPerdecompRows(rows) {
  ...
  // Note: Unlike other append functions, this one expects 'rows' to be a 2D array
  // already mapped to the correct column order.
  // The validation and mapping should happen in the calling API route.
  return sheets.spreadsheets.values.append({
    spreadsheetId: process.env.SPREADSHEET_ID,
    range: PERDECOMP_SHEET_NAME,
    valueInputOption: 'USER_ENTERED',
