...
// ✅ Mapeamento para aba PERDECOMP
const PERDECOMP_COLUMN_MAP = {
    Cliente_ID: 'Cliente_ID',
    Nome_da_Empresa: 'Nome da Empresa',
    ...
    Quantidade_DARFs: 'Quantidade_DARFs',
...
// ---------- Funções para a aba PERDECOMP ----------

async function fetchPerdecompHeaderInfo(spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId,
...
export async function getPerdecompHeaderInfoCached(
  spreadsheetId = process.env.SPREADSHEET_ID
) {
  const key = `header-perdecomp:${spreadsheetId}`;
  return withCache(key, () => fetchPerdecompHeaderInfo(spreadsheetId));
}

async function fetchPerdecompSheet(spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  return sheets.spreadsheets.values.get({
    spreadsheetId,
...
export async function getPerdecompSheetCached(
  spreadsheetId = process.env.SPREADSHEET_ID
) {
  const key = `sheet-perdecomp:${spreadsheetId}`;
  return withCache(key, () => fetchPerdecompSheet(spreadsheetId));
}

export async function appendPerdecompRows(rows) {
    const auth = await authorize();
    const sheets = google.sheets({ version: 'v4', auth });

    const orderedColumnKeys = Object.keys(PERDECOMP_COLUMN_MAP);

    const values = rows.map(rowObject => {
        return orderedColumnKeys.map(key => {
            const value = rowObject[key] !== undefined ? rowObject[key] : '';
