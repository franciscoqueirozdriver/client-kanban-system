...
// Funções genéricas para outras abas
export async function getSheetByName(sheetName, spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  return sheets.spreadsheets.values.get({
    spreadsheetId,
...
export async function getSheetByNameCached(sheetName, spreadsheetId = process.env.SPREADSHEET_ID) {
  const key = `sheet:${spreadsheetId}:${sheetName}`;
  return withCache(key, () => getSheetByName(sheetName, spreadsheetId));
}

export async function appendRows(sheetName, rows, spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  const protectedRows = rows.map((row) => row.map((v) => protectValue(v)));
  return sheets.spreadsheets.values.append({
    spreadsheetId,
    range: sheetName,
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: protectedRows },
  });
}
