...
export async function appendCompanyImportRow(empresa, spreadsheetId = process.env.SPREADSHEET_ID) {
  ...
  // 1) Ler cabeçalho (linha 1)
  const headerRes = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: `${COMPANY_IMPORT_SHEET_NAME}!1:1`,
  });
  const header = (headerRes.data?.values?.[0] || []).map(String);

  // 2) Montar mapa chave->valor usando os títulos EXATOS da planilha (com acentos)
  const map = {
  ...
  // 3) Produzir a linha final respeitando a ORDEM do cabeçalho lido
  const row = header.map((col) => (col in map ? String(map[col]) : ''));

  // 4) Append (sem depender de "updates")
  const appendRes = await sheets.spreadsheets.values.append({
    spreadsheetId,
    range: `${COMPANY_IMPORT_SHEET_NAME}!A1`,
    valueInputOption: 'USER_ENTERED',
    insertDataOption: 'INSERT_ROWS',
    requestBody: { values: [row] },
  });

  // Opcional: retornar metadados úteis e a contagem atual de linhas
  const countRes = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: `${COMPANY_IMPORT_SHEET_NAME}!A:A`,
  });
