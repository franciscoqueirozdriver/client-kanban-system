...
export async function _findRowNumberByClienteId(sheetName, clienteId) {
    ...
    const rowIndex = rows.findIndex(row => row.cliente_id === clienteId); // Corrigido para snake_case
...
// --- Missing Helpers Restored from Legacy/Conflict ---
...
export async function getCompanySheetCached() {
  // Wrapper for compatibility with lib/perplexity.js
  return getSheetCached('layout_importacao_empresas');
}

export async function getCompanySheet() {
  // Wrapper for compatibility with pages/api/exportCompanies.js
  return getSheetCached('layout_importacao_empresas');
}

export async function appendCompanyImportRow(data) {
  const sheetName = 'layout_importacao_empresas';
  // We need headers to map the data correctly
  ...
  Object.entries(data).forEach(([key, value]) => {
    // key comes from the object keys (e.g., 'nome', 'site')
    // COMPANY_COLUMN_MAP maps 'nome' -> 'nome' (or whatever is in the map)
    // Use the map to find the target header name
    const targetHeaderKey = COMPANY_COLUMN_MAP[key];
    if (targetHeaderKey && headerMap[targetHeaderKey] !== undefined) {
      row[headerMap[targetHeaderKey]] = protectValue(value);
    }
  });
  ...
  // Return the full response so the caller can read updatedRange
  return withRetry(() =>
