...
// Requer: cliente Sheets autenticado via getSheetsClient().
// Uso: append linhas ao final do range informado (A1 notation).
export async function appendSheetData(optionsOrSheetName, maybeValues) {
  // Compatibilidade com a assinatura antiga: (sheetName: string, values: any[][])
  if (typeof optionsOrSheetName === 'string') {
    return appendSheetDataLegacy(optionsOrSheetName, maybeValues);
  }
  ...
  if (!Array.isArray(values)) throw new Error('appendSheetData: values deve ser array de arrays');
