...
const HISTORY_COLUMN_MAP = {
  cliente_id: 'Cliente_ID',
  data_hora: 'Data/Hora', // Corrigido de 'Data_Hora'
  tipo: 'Tipo',
  de_fase: 'De_Fase',
  para_fase: 'Para_Fase',
  canal: 'Canal',
  observacao: 'Observação', // Corrigido de 'Observacao'
  mensagem_usada: 'Mensagem', // Corrigido de 'Mensagem_Usada'
};
...
async function authorize() {
  try {
    ...
    console.error('Falha na autorização com a API do Google:', error);
    throw new Error('Falha na autorização com o Google. Verifique as credenciais.');
  }
}
...
export async function appendRow(data) {
  try {
    ...
    console.error('Erro ao adicionar linha na planilha principal:', error);
    throw error;
  }
}
...
export async function appendHistoryRow(data) {
  try {
    ...
    console.error('Erro ao adicionar linha na planilha de histórico:', error);
    throw error;
  }
}
...
export async function batchUpdateRows(updates) {
  try {
    ...
    console.error('Erro ao atualizar linhas em lote na planilha:', error);
    throw error;
  }
}

export async function updateRow(rowNumber, data) {
  return batchUpdateRows([{ rowNumber, data }]);
}

// ✅ Função de agregação com Cliente_ID para relatórios (REMOVIDA)
// A lógica foi centralizada em lib/dataUtils.js na função aggregateClientData.
