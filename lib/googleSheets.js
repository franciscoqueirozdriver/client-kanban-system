...
/**
 * Updates a specific row in a given sheet with new data.
 * This is a more robust version that reads the row, updates in memory, and writes back.
 * @param {string} sheetName The name of the sheet.
 * @param {number} rowNumber The 1-based row number to update.
 * @param {object} newData An object where keys are column headers and values are the new cell values.
 */
export async function updateRowData(sheetName, rowNumber, newData) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });

  // 1. Get headers for the target sheet
  ...
  if (!headers) {
    throw new Error(`Could not find headers for sheet: ${sheetName}`);
  }
  const headerMap = new Map(headers.map((h, i) => [h, i]));

  // 2. Read the existing row data
  const range = `${sheetName}!A${rowNumber}:${columnToLetter(headers.length - 1)}${rowNumber}`;
  const rowRes = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.SPREADSHEET_ID,
    range,
  });
  const rowValues = rowRes.data.values?.[0] || new Array(headers.length).fill('');

  // 3. Update the values in memory
  for (const [colName, value] of Object.entries(newData)) {
    const colIndex = headerMap.get(colName);
    if (colIndex !== undefined) {
      rowValues[colIndex] = protectValue(value);
    }
  }

  // 4. Write the entire updated row back
