...
export async function getNextClienteId() {
  ...
  return `CLI-${String(maxId + 1).padStart(6, '0')}`;
}
...
function _columnToLetter(index) {
  let letter = '', temp = index;
  while (temp >= 0) {
...
export async function updateRow(rowNumber, data) {
    ...
    for (const key in data) {
        ...
        if (colIndex !== undefined) {
            const colLetter = _columnToLetter(colIndex);
...
export async function updateInSheets(clienteId, payload) {
    ...
    for (const sheetName in sheetLayouts) {
        ...
        const rangeEndColumn = String.fromCharCode(64 + sheetLayouts[sheetName].cols);
