...
import { requireEnv, normalizedGoogleKey } from "./env.js";
...
async function getSheetsClient() {
  if (!sheetsClientPromise) {
    sheetsClientPromise = (async () => {
      requireEnv(['GOOGLE_CLIENT_EMAIL', 'GOOGLE_PRIVATE_KEY', 'SPREADSHEET_ID']);

      const auth = new google.auth.JWT({
        email: process.env.GOOGLE_CLIENT_EMAIL,
        key: normalizedGoogleKey(),
...
export async function getSheetData(sheetName, range = 'A:ZZ', spreadsheetId = process.env.SPREADSHEET_ID, bustCache = false) {
...
// --- Spreadsheet Metadata & Structure Helpers ---

export async function getSpreadsheet(spreadsheetId = process.env.SPREADSHEET_ID) {
  const sheets = await getSheetsClient();
  const res = await withRetry(() =>
...
export async function getHeader(sheetTitle, spreadsheetId = process.env.SPREADSHEET_ID) {
  const sheets = await getSheetsClient();
  const res = await withRetry(() =>
    sheets.spreadsheets.values.get({
      spreadsheetId,
...
export async function createSheetIfMissing(title, spreadsheetId = process.env.SPREADSHEET_ID) {
...
export async function appendMissingColumns(sheetTitle, targetHeader, spreadsheetId = process.env.SPREADSHEET_ID) {
  const currentHeader = await getHeader(sheetTitle, spreadsheetId);
  const missing = targetHeader.filter(h => !currentHeader.includes(h));
