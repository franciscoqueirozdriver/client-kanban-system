...
const MESSAGE_LOG_SHEET_NAME = 'Mensagens';
...
// ✅ Mapeamento de colunas da aba de mensagens/histórico
const MESSAGE_COLUMN_MAP = {
...
// ---------- Funções para registrar mensagens / histórico ----------

async function fetchMessageHeaderInfo(spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: `${MESSAGE_LOG_SHEET_NAME}!1:1`,
  });
  const headers = res.data.values ? res.data.values[0] : [];
  const indexMap = {};
  headers.forEach((h, i) => {
...
export async function getMessageHeaderInfoCached(
  spreadsheetId = process.env.SPREADSHEET_ID
) {
  const key = `header-message:${spreadsheetId}`;
  return withCache(key, () => fetchMessageHeaderInfo(spreadsheetId));
}

async function fetchMessageSheet(spreadsheetId = process.env.SPREADSHEET_ID) {
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  return sheets.spreadsheets.values.get({
    spreadsheetId,
    range: MESSAGE_LOG_SHEET_NAME,
  });
}

export async function getMessageSheetCached(
  spreadsheetId = process.env.SPREADSHEET_ID
) {
  const key = `sheet-message:${spreadsheetId}`;
  return withCache(key, () => fetchMessageSheet(spreadsheetId));
}

export async function generateNextMessageId() {
  ...
  const header = rows[0];
  const idx = header.indexOf('Message_ID');
  if (idx === -1) return `MSG-${String(Date.now())}`;
  const ids = rows.slice(1)
    .map(r => r[idx] || '')
    .map(id => parseInt(String(id).replace(/\D/g, ''), 10))
    .filter(n => !isNaN(n));
  const next = ids.length ? Math.max(...ids) + 1 : 1;
  return `MSG-${String(next).padStart(5, '0')}`;
}

export async function appendMessageRow(data) {
  const { headers, indexMap } = await getMessageHeaderInfoCached();
  const auth = await authorize();
  const sheets = google.sheets({ version: 'v4', auth });
  const row = new Array(headers.length).fill('');
  Object.entries(data).forEach(([key, value]) => {
    const header = MESSAGE_COLUMN_MAP[key];
    if (header && indexMap[header] !== undefined) {
      row[indexMap[header]] = protectValue(value);
    }
  });
