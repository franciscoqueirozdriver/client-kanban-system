...
export function mapHeadersToSnakeCase(sheetName, originalHeaders) {
    ...
    return originalHeaders.map(h => snakeCaseMap[h.trim()] || h.toLowerCase().replace(/ /g, '_'));
}
...
/**
 * Converte uma linha crua da planilha em um objeto com chaves snake_case.
 * @param {string} sheetName - O nome da aba (snake_case).
 * @param {string[]} headers - Array de nomes de colunas originais.
 * @param {any[]} row - Array de valores da linha.
 * @returns {object} Objeto com chaves em snake_case.
 */
export function mapSheetRowToSnakeObject(sheetName, headers, row) {
    const mapping = getColumnMapping(sheetName);
    const snakeObject = {};
    headers.forEach((header, index) => {
        const snakeKey = mapping[header];
        if (snakeKey) {
            snakeObject[snakeKey] = row[index];
        }
    });
    return snakeObject;
}
