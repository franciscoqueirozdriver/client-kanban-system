    ...
    Credentials({
      ...
      authorize: async (creds, req) => {
        try {
          ...
          // New check: if user exists but has no password, throw a special error.
          // A hash is long, so any short or empty string is considered "no password".
          if (!user.Hash_Senha || user.Hash_Senha.trim().length < 10) {
            // DEBUG: Throw the actual value to see what the system is reading.
            throw new Error(`DEBUG: NO_PASSWORD. Hash value is: [${user.Hash_Senha}]`);
          }
