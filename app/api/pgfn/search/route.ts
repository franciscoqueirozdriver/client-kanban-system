...
export async function GET(req: NextRequest) {
  ...
  const tempo = searchParams.get('tempo') || 'all';
  const aju = searchParams.get('aju') || 'all';
  ...
  if (q) {
    where.push(`(\n      d.numero_inscricao ILIKE $${++paramIndex} OR\n      d.nome_devedor     ILIKE $${paramIndex} OR\n      d.cpf_cnpj         ILIKE $${paramIndex}\n    )`);
    params.push(`%${q}%`);
  }
